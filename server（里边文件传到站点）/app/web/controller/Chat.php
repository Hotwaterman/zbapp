            ...
            if ($prompt_id) {
                ...
                if (!empty($role['prompt'])) {
                    if ($channel == 'baidu') {
                        $question[] = [
                            'role' => 'user',
                            'content' => '请你遵从以下指令：' . trim($role['prompt'])
                        ];
                        $question[] = [
                            'role' => 'assistant',
                            'content' => '好的'
                        ...
                        $question[] = [
                            'role' => 'system',
                            'content' => trim($role['prompt']) . '。' . $systemPrompt
                        ];
                    }
                }
...
6.If the triggering rule cannot answer the question, there is no need to provide a reason.';
            ...
            if ($channel == 'baidu') {
                ...
                $apikey = $temp[0];
                $secretkey = $temp[1] ?? '';
                $SDK = new \Baidu\wenxin($apikey, $secretkey, $temperature);
        ...
        foreach ($rows as $data) {
            ...
            if (strpos($data, 'data: [DONE]') !== false) {
                ...
                if (isset($data['choices'])) {
                    ...
                    // baidu data
                    $char = $data['result'];
                    if ($data['is_end'] || $data['need_clear_history']) {
                        $is_end = true;
                    }
                }
            }
    ...
    private function getBookContent($books, $text)
    {
        ...
        foreach ($list as $v) {
            ...
            // file_put_contents('./a.txt', $v['title'] . $distance. "\n", 8);
