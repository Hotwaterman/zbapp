...
class lxai
{
    ...
    protected static $apiHost = 'http://ai.4387.top:8606';
    ...
    {
        $url = 'http://ai.4387.top:8010/v1/chat/completions';
        $post = [
            'messages' => $message,
    ...
    {
        $url = self::$apiHost . '/mj/trigger/submit';
        $post = [
            'action' => 'IMAGINE',
    ...
    {
        $url = self::$apiHost . '/mj/trigger/submit-uv';
        $post = [
            'id' => $option['id'],
            'type' => $option['type'],
            'index' => $option['index'],
    ...
    public function splitMjImage($imageUrl)
    {
        ...
        imagejpeg($newImage, $filepath4, 80);
    ...
    {
        $url = self::$apiHost . '/mj/trigger/upload';
        ...
        file_put_contents('./lxupload.txt', date('Y-m-d H:i:s') . "\n", 8);
        file_put_contents('./lxupload.txt', $fileName . "\n", 8);
        file_put_contents('./lxupload.txt', $url . "\n", 8);
        file_put_contents('./lxupload.txt', json_encode($header) . "\n", 8);
        file_put_contents('./lxupload.txt', $data2 . "\n", 8);
//        file_put_contents('./lxupload.txt', $data . "\n", 8);
    ...
    {
        $url = self::$apiHost . '/mj/task/' . $task_id . '/fetch';
    ...
    private function httpUpload($url, $header, $data = '')
    {
        ...
        $ret = curl_exec($ch);
        file_put_contents('./lxupload.txt', $ret . "\n\n\n", 8);
        if (false === $ret) {
