      ...
      >
        ...
        <el-table-column prop="total_tokens" label="消耗tokens" width="100">
          <template slot-scope="scope">
            <span v-if="scope.row.user === 'AI' && scope.row.total_tokens > 0">{{ scope.row.total_tokens }}</span>
          </template>
        </el-table-column>
        <el-table-column label="用户" width="100">
          <template slot-scope="scope">
            <p style="text-align: right;">
              <span v-if="scope.row.user !== 'AI'" style="font-weight: bold;">客户{{ scope.row.user_id }}：</span>
              <span v-else>AI：</span>
            </p>
          </template>
        </el-table-column>
        <el-table-column label="内容">
          <template slot-scope="scope">
            <p :style="'font-weight:' + (scope.row.user !== 'AI' ? 'bold' : 'normal')" style="margin: 10px 0;" v-html="scope.row.message" />
...
<script>
import { getOldMsgList, getOldMsgTongji } from '@/api/chat'
export default {
  data() {
    return {
      dataList: [],
